---
title: "Getting Started with LegacyNNS: Sampling and Simulation"
author: "Fred Viole"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with LegacyNNS: Sampling and Simulation}
...
`LegacyNNS` offers several novel sampling methods from any distribution, as well as simulating variables while maintaining their dependence.
...
## Bootstrapping (**`LegacyNNS.meboot`**)

**`LegacyNNS.meboot`** is based on the maximum entropy bootstrap, available in the R-package `meboot`. This procedure is specifically designed for time-series and avoids the IID assumption in traditional methods.
...
We will generate 1 replicate of $X$ for each value of a sequence of $\rho$ values, and then plot the results compared to our original $X$ (black line). **`LegacyNNS.MC`** is a streamlined wrapper function for this functionality of **`LegacyNNS.meboot`**.
...
![](images/LegacyNNSmc_1_tgt_drift.png)

Please see the full **`LegacyNNS.meboot`** and **`LegacyNNS.MC`** argument documentation.
...
## Alternative Using **`LegacyNNS.meboot`**

Alternatively, if we wish to keep the simulated values close to the original data, we can apply the **`LegacyNNS.meboot`** procedure to each of the variables.
