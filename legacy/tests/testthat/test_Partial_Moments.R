...
R1 <- LegacyNNS::PM.matrix(1,1,colMeans(A), A, pop_adj = TRUE)$cov.matrix
R2 <- LegacyNNS::PM.matrix(1,1,colMeans(A), A, pop_adj = FALSE)$cov.matrix
test_that(
  "LegacyNNS::PM.matrix - Mean Target", {
...
R1 <- LegacyNNS::PM.matrix(1,1,NULL, A, pop_adj = TRUE)$cov.matrix
R2 <- LegacyNNS::PM.matrix(1,1,NULL, A, pop_adj = FALSE)$cov.matrix
test_that(
  "LegacyNNS::PM.matrix - NULL Target", {
    expect_equal(T1, cov(A), tolerance=1e-5)
    expect_equal(R1, T1, tolerance=1e-5)
    expect_equal(R2, T2, tolerance=1e-5)
  }
...
R2 <- LegacyNNS::PM.matrix(1,1,colMeans(A), A, pop_adj = FALSE)$cov.matrix
test_that(
  "LegacyNNS::PM.matrix - Mean Target - DataFrame", {
...
R2 <- LegacyNNS::PM.matrix(1,1,NULL, A, pop_adj = FALSE)$cov.matrix
test_that(
  "LegacyNNS::PM.matrix - NULL Target - DataFrame", {
    expect_equal(R1, T1_n, tolerance=1e-5)
    expect_equal(R2, T2_n, tolerance=1e-5)
  }
)
...
colnames(T1) <- c("A", "S(x)")
B<-LegacyNNS.CDF(A, type="survival")
test_that(
  "LegacyNNS.CDF", {
