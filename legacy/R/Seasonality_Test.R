...
#' @return Returns a matrix of all periods exhibiting less coefficient of variation than the variable with \code{"all.periods"}; and the single period exhibiting the least coefficient of variation versus the variable with \code{"best.period"}; as well as a vector of \code{"periods"} for easy call into \link{LegacyNNS.ARMA.optim}.  If no seasonality is detected, \code{LegacyNNS.seas} will return ("No Seasonality Detected").
...
#' LegacyNNS.seas(x, plot = FALSE)$best.period
#'
#' ## Using modulos for logical seasonal inference:
#' LegacyNNS.seas(x, modulo = c(2,3,5,7), plot = FALSE)
#' }
#' @export


LegacyNNS.seas <- function(variable,
                     modulo = NULL,
                     mod.only = TRUE,
                     plot = TRUE){
