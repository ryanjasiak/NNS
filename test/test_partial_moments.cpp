...
TEST_CASE("Version 2: NNS::partial_moment - 1")
{
    // Get the R inside singleton
    RInside &R = NNS_test::rinside::r();
    ...
    // Create sample data
    std::vector<double> v = {5.0, 2.5, 2.2, 3.8};
    Eigen::VectorXd e_v = Eigen::Map<Eigen::VectorXd>(v.data(), v.size());
    Rcpp::NumericVector r_v(v.begin(), v.end());
    R["v"] = r_v;

    double degree = 3.0; // e.g. third-order moment
    double target = 6.0; // threshold/benchmark
    R["d"] = degree;
    R["t"] = target;
...
TEST_CASE("Version 2: NNS::co_partial_moment - 1")
{
    // R instance --------------------------------------------------------------
    RInside &R = NNS_test::rinside::r();
    ...
    // sample data -------------------------------------------------------------
    Eigen::VectorXd x(5);
    x << 0.05, 0.02, 0.03, -0.02, 0.07;
    Eigen::VectorXd y(5);
    y << 0.03, 0.01, 0.02, -0.04, 0.05;

    double k  = 3.0;   // moment degree
...
TEST_CASE("Version 2: NNS::divergent_partial_moment - 1")
{
    // R instance --------------------------------------------------------------
    RInside &R = NNS_test::rinside::r();
    ...
    // sample data -------------------------------------------------------------
    Eigen::VectorXd x(5);
    x << 0.02, 0.07, 0.04, -0.03, 0.01;
    Eigen::VectorXd y(5);
    y << 0.02, 0.04, 0.03, -0.04, 0.05;
